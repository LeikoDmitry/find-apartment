...
- `telegram_command_watcher.py` — слушает Telegram (long polling) и мгновенно
  применяет команды `цена <min> <max>` / `комнаты <list>`. Должен работать в
  единственном экземпляре, см. [«Только один опрашивающий на бота»](#только-один-опрашивающий-на-бота).
...
### Только один опрашивающий на бота

Telegram отдаёт `getUpdates` ровно одному вызывающему: как только приходит
второй запрос по тому же токену, первый выбивается с `409 Conflict`. Два
watcher'а на один токен будут бесконечно перехватывать опрос друг у друга, и
команды начнут доходить через раз. Поэтому одновременно должен работать ровно
один опрашивающий процесс — либо сервис `watcher`, либо ручной запуск
`telegram_command_watcher.py`, либо `check_telegram_commands.py`, но не
несколько сразу.

Чтобы это не приходилось ловить руками, `telegram_command_watcher.py` при
старте берёт эксклюзивный `flock` на `watcher.lock` в корне проекта и, если
блокировку уже держит другой watcher, сразу выходит с сообщением и PID
владельца. Директория проекта монтируется в контейнер (`.:/app`), так что
локальный запуск и контейнер видят один и тот же файл блокировки.

Через `flock` не отлавливается только случай, когда копия развёрнута на другой
машине (например, после переезда на новый сервер старый экземпляр забыли
погасить) — там watcher напишет в лог `409 Conflict` и будет повторять попытку
раз в минуту, пока лишний экземпляр не остановят. Проверить, опрашивает ли бота
кто-то ещё, можно так — при остановленном watcher'е:

```bash
TOKEN=$(grep '^bot_token:' config.yaml | awk '{print $2}' | tr -d "\"'")
curl -s "https://api.telegram.org/bot$TOKEN/getUpdates?timeout=25"
```

`"ok":true` — конкурентов нет; `"error_code":409` — где-то работает второй
